/**
 * @fileoverview Unit tests cho trang Quản lý khách mời
 * @description Test các chức năng CRUD, edit modal với ghế ngồi và món ăn
 * @version 1.0.0
 * @since 2025-10-02
 * @author Dũng Đàm
 */
...
import QuanLyKhachMoiPage from '@/app/quan-ly-khach-moi/page'
import { getPersonsPaginated, deletePerson, getPersonByEmail, registerOrUpdatePerson } from '@/services/person.service'
...
jest.mock('next/navigation', () => ({
  ...
  usePathname: jest.fn(() => '/quan-ly-khach-moi'),
}))
...
const mockGetPersonsPaginated = getPersonsPaginated as jest.MockedFunction<typeof getPersonsPaginated>
const mockDeletePerson = deletePerson as jest.MockedFunction<typeof deletePerson>
const mockGetPersonByEmail = getPersonByEmail as jest.MockedFunction<typeof getPersonByEmail>
const mockRegisterOrUpdatePerson = registerOrUpdatePerson as jest.MockedFunction<typeof registerOrUpdatePerson>
const mockGetItems = getItems as jest.MockedFunction<typeof getItems>
const mockGetSeatsInfo = getSeatsInfo as jest.MockedFunction<typeof getSeatsInfo>

describe('QuanLyKhachMoiPage', () => {
  const mockPersons = [
    {
      personId: '1',
  ...
  beforeEach(() => {
    ...
    mockGetPersonsPaginated.mockResolvedValue({
      content: mockPersons,
      page: 0,
      size: 10,
      totalElements: 2,
  ...
  /**
   * @test Kiểm tra render danh sách khách mời
   */
  it('should render list of persons successfully', async () => {
    render(<QuanLyKhachMoiPage />)
  ...
  it('should filter persons by search term', async () => {
    ...
    const searchInput = screen.getByPlaceholderText(/Tìm kiếm theo tên, email, chức vụ/i)
  ...
  it('should open edit modal and fetch full person data', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Chỉnh sửa thông tin khách mời')).toBeInTheDocument()
    })
  })
  ...
  it('should show seat selection for person without seat', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Chỉ hiển thị ghế trống chưa ai đặt')).toBeInTheDocument()
    })
  })
  ...
  it('should only send changed fields when updating', async () => {
    ...
    mockGetPersonByEmail.mockResolvedValue(mockPersons[0])
    mockRegisterOrUpdatePerson.mockResolvedValue({ code: 200, message: 'Success' })
  ...
  it('should delete person successfully', async () => {
    ...
    await waitFor(() => {
      ...
      expect(toast.success).toHaveBeenCalledWith('Đã xóa khách mời "Nguyễn Văn A".')
    })
  })
  ...
  it('should show info toast when no changes detected', async () => {
    ...
    await waitFor(() => {
      expect(toast.info).toHaveBeenCalledWith('Không có thay đổi nào để cập nhật')
