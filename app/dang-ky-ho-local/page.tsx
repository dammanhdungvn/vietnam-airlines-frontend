...
/**
 * Trang đăng ký hộ Local (SUPERADMIN) - Quy trình đăng ký khách hàng tham gia sự kiện
 * Bao gồm 4 bước: Chọn khách hàng, Lấy thông tin, Chọn ghế, Khu trải nghiệm
 */
export default function DangKyHoLocalPage() {
  ...
  const [personsResponse, setPersonsResponse] =
    useState<PaginatedApiResponse<Person> | null>(null);
  ...
  // Hàm tải danh sách khách hàng (sử dụng API superadmin-created)
  const fetchPersonsFull = async () => {
    setIsLoading(true);
    try {
      const initial = await getPersonsSuperAdminCreated({
        page: 0,
        size: 1,
        sortBy: "personId",
        sortDir: "asc",
  ...
  useEffect(() => {
    if (selectedCustomer?.avatarUrl) {
      ...
      const normalized =
        isDataUrl || isHttpUrl ? raw : `data:image/jpeg;base64,${raw}`;
  ...
  const fetchSeats = async () => {
    ...
    try {
      ...
      const initialData = await getSeatsInfo({
        page: 0,
        size: 1,
        sortBy: "id",
        sortDir: "asc",
      ...
      if (totalElements > 0) {
        ...
        const allSeatsData = await getSeatsInfo({
          page: 0,
          size: totalElements,
          sortBy: "id",
          sortDir: "asc",
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      if (totalElements > 0) {
        const allItemsData = await getItems({
          page: 0,
          size: totalElements,
          sortBy: "id",
          sortDir: "asc",
  ...
  useEffect(() => {
    ...
    const sliced = base.slice(
      safePage * clientSize,
      safePage * clientSize + clientSize
    );
    setPersonsResponse({
      content: sliced,
      page: safePage,
  ...
  const steps = [
    {
      number: 1,
      title: "Khách hàng",
      subtitle: "Chọn từ danh sách",
      icon: User,
    },
    {
      number: 2,
      title: "Lấy thông tin",
      subtitle: "Face ID để tham gia",
      icon: Info,
    },
    {
      number: 3,
      title: "Ghế ngồi",
      subtitle: "Chọn vị trí trong hội nghị",
      icon: Plane,
    },
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      if (!selectedCustomer) {
        toast({
          title: "Vui lòng chọn một khách hàng",
          variant: "destructive",
        });
        return;
      }
    ...
    if (currentStep === 2) {
      ...
      if (!hasExistingImage && !hasNewImage) {
        setError({
          ...
          message:
            "Khách hàng chưa có ảnh nhận diện. Vui lòng tải lên ảnh Face ID để tiếp tục.",
        });
        return;
      }
      ...
      try {
        ...
        if (hasNewImage) {
          const response = await validateAndUploadFace(
    ...
    if (currentStep < 4) {
      ...
      const mergedSeatInfo = selectedSeat
        ? {
            ...
            paidPrice: selectedSeat.basePrice ?? 0,
          }
        : selectedCustomer.seatInfo
        ? {
            seatNumber: selectedCustomer.seatInfo.seatNumber,
            paidPrice: selectedCustomer.seatInfo.paidPrice ?? 0,
          }
        : null;
      ...
      try {
        ...
        const safePayload = {
          ...fullPayload,
          ...(fullPayload.gender
            ? { gender: String(fullPayload.gender).toUpperCase() }
            : {}),
          ...(typeof fullPayload.status === "boolean"
            ? { status: Boolean(fullPayload.status) }
            : {}),
        };
  ...
  const handleComboConfirm = (quantity: number) => {
    if (selectedCombo) {
      setCombos((prev) =>
        prev.map((combo) =>
          combo.id === selectedCombo.id ? { ...combo, quantity } : combo
        )
      );
    }
  };
            ...
            <CustomerList
              ...
              onPageChange={(page) => {
                ...
                setPersonsResponse({
                  ...
                  last:
                    page >=
                    Math.max(1, Math.ceil(base.length / clientSize)) - 1,
                });
              }}
              onSearch={(keyword) => {
          ...
          <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6">
            ...
            <div className="space-y-3 sm:space-y-4">
              <h3 className="text-base sm:text-lg font-medium">
                Đăng ký nhận diện tự động
              </h3>
              <p className="text-xs sm:text-sm text-gray-600">
                * Ảnh được dùng để hỗ trợ nhận diện khi bạn đến Hội nghị, giúp
                thủ tục check-in diễn ra nhanh và thuận tiện hơn. Thông tin sẽ
                được quản lý an toàn và được xóa bỏ ngay khi sự kiện kết thúc.
              </p>
  ...
  const isNextButtonDisabled = () => {
    ...
    if (currentStep === 2) {
      const hasAnyImage =
        Boolean(faceIdImage) || Boolean(selectedCustomer?.avatarUrl);
    ...
    <PageContainer className="p-4 sm:p-6 md:p-8">
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">
          Đăng ký hộ khách tại sự kiện - SUPERADMIN
        </p>
      </div>
