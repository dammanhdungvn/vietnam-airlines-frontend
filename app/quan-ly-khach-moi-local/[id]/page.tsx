...
/**
 * Trang chi tiết và chỉnh sửa khách mời (Local - SUPERADMIN)
 * Hiển thị thông tin chi tiết của một khách mời và cho phép chỉnh sửa.
 */
export default function ChiTietKhachMoiLocalPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const finalSeatNumber =
        formData.seatInfo?.seatNumber || person.seatInfo?.seatNumber;
      const finalSeatInfo = finalSeatNumber
        ? {
            seatNumber: finalSeatNumber,
            paidPrice:
              formData.seatInfo?.paidPrice ?? person.seatInfo?.paidPrice ?? 0,
          }
        : null;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push("/quan-ly-khach-moi-local");
    } catch (error) {
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Chi tiết khách mời (Local)
            </h1>
            <p className="text-sm text-gray-500">
              {isEditMode
                ? `Chỉnh sửa thông tin của ${person.fullName}`
                : `Thông tin chi tiết của ${person.fullName}`}
            </p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Bạn có chắc chắn muốn xóa khách mời này? Hành động này
                      không thể hoàn tác.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                ...
                <div className="relative">
                  <Avatar className="w-24 h-24">
                    ...
                    <AvatarFallback className="text-3xl">
                      {getAvatarFallback(person.fullName)}
                    </AvatarFallback>
                  </Avatar>
                  {isEditMode && (
                    <Button
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <InfoField
                  ...
                  isEditMode={isEditMode}
                  onChange={(e) =>
                    handleSeatInfoChange("seatNumber", e.target.value)
                  }
                  formDataValue={formData.seatInfo?.seatNumber}
                />
                <InfoField
                  label="Giá vé đã trả"
                  value={(person.seatInfo?.paidPrice ?? 0).toLocaleString(
                    "vi-VN",
                  ...
                  type="number"
                  onChange={(e) =>
                    handleSeatInfoChange("paidPrice", Number(e.target.value))
                  }
            ...
            <CardContent>
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm font-medium">
                    {new Date(person.createdAt).toLocaleString("vi-VN")}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Cập nhật lần cuối</p>
                  <p className="text-sm font-medium">
                    {new Date(person.updatedAt).toLocaleString("vi-VN")}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Loại khách</p>
                  <p className="text-sm font-medium">
                    {person.isVip ? "VIP" : "Thường"}
                  </p>
                </div>
              </div>
            </CardContent>
...
const InfoField = ({
  ...
  <div className="flex items-center space-x-3">
    {icon && (
      <div className="w-4 h-4 text-gray-400 mt-3 self-start">{icon}</div>
    )}
    <div className="w-full">
